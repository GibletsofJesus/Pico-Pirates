...
state=1--change to 1 to skip intro
--0 splash screen
--1 gameplay
--2 screen transition
	...
	if state==1 then
	  ...
	  boat_draw(boat)
		...
		if map then
			...
			for x=1,#cells do
				for y=1,#cells[x] do
					if (cells[x][y].seed>island_prob) circfill(camx+x*2,camy+y*2,0,15) flipp()
					if (cells[x][y].seed<wp_prob) circfill(camx+x*2,camy+y*2,0,7) flipp()
				end
			end
			if (flr(t()*4)%2>0) pset(camx+currentcellx*2,camy+currentcelly*2,4) flipp()
		end
...
function minimap()
	...
	if (cells[currentcellx][currentcelly].seed>island_prob) circfill(camx+119,camy+8,island.size/16,15) flipp()
	if cells[currentcellx][currentcelly].seed<wp_prob then
		fillp(fillps[1])
		circfill(camx+119,camy+8,4,7) flipp()
		fillp()
	end
	pset(camx+112+flr(((boat.x+256)/512)*14),camy+1+flr(((boat.y+256)/512)*14),4) flipp()
...
island={
	...
	draw=function()
		...
		if (island.size > 8) circfill(0,0,island.size*.8,6) flipp()
		if (island.size > 16) circfill(0,0,island.size*.5,4) flipp()

		fillp(fillps[2],-camx,-camy)
		if (island.size > 6) circfill(0,0,island.size*1.35,6) flipp()
		fillp(fillps[4],-camx,-camy)
		if (island.size > 16) circfill(0,0,island.size*.35,9) flipp()
		fillp()
		--draw trees
		for t in all(island.trees) do
			if(t.c<2)t.draw(t)
